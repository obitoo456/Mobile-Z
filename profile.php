...
$data = getSpeData("user","user_id",$id);
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        if(exists("user","user_id",$id)>0){
            ...
            $stmt = $con->prepare('UPDATE `user` SET `f_name`=?,`l_name`=?,`email`=?,`password`=?,`img`=? WHERE `user_id` = ?');
            $stmt->execute(array($f_name,$l_name,$email,$pass,$img,$id));
            if($stmt){
...
<h2>Profile</h2>
    <div class="container">
        ...
        <label for="">Chose Your Avatar</label>
            ...
            <input type="hidden" name="img" value="/avatars/avatar1.svg">  
        </div>
